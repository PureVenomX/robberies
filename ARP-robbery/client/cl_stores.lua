...
AddEventHandler("ARP-jobmanager:playerBecameJob", function(job, name, notify)
...
local SecurityCamLocations = {
	...
	[16] =  { ['x'] = 1697.8,['y'] = 4922.69,['z'] = 42.07,['h'] = 322.95, ['info'] = ' Grapeseed Main - 16', ["recent"] = false },
	[17] =  { ['x'] = 1728.82,['y'] = 6417.38,['z'] = 35.04,['h'] = 233.94, ['info'] = ' North Senora / Pelato - 17', ["recent"] = false },
	[18] =  { ['x'] = 237.49,['y'] = 219.81,['z'] = 110.28,['h'] = 268.55, ['info'] = ' Pacific Bank Camera 1 - 18', ["recent"] = false },
	[19] =  { ['x'] = 258.42,['y'] = 224.16,['z'] = 103.23,['h'] = 49.24, ['info'] = ' Pacific Bank Camera 2 - 19', ["recent"] = false },
	[20] =  { ['x'] = -637.40,['y'] = -250.91,['z'] = 45.29,['h'] = 330.90, ['info'] = ' Vagelico Outside Camera 1 - 20', ["recent"] = false },
	[21] =  { ['x'] = -615.22,['y'] = -230.60,['z'] = 39.24,['h'] = 85.91, ['info'] = ' Vagelico Inside Camera 2 - 21', ["recent"] = false },
}
...
AddEventHandler("police:notifySecurityCam", function(currentRobbery)
	local myjob = exports["ARP_manager"]:isPed("myjob")
...
AddEventHandler("store:dosafe", function()
	...
	if police >= 2 then
		...
		TriggerEvent("ARP-dispatch:storerobbery")
...
AddEventHandler("store:register", function(storeid,regid)
	...
	if police >= 2 then
		...
		TriggerServerEvent("police:camrobbery",storeid)
		TriggerEvent("client:newStress",true,200)
		TriggerEvent("safecracking:loop",5,"robbery:register")
		store_id = storeid
		register_id = regid
		isLockpicking = true
	else
	...
	if not IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		if storeid ~= 0 then
			...
			if #(GoodSpot - curCoords) > 1.0 then
				TriggerEvent("DoLongHudtext","You must be at the front of the register")
				isLockpicking = false
				return
			end
			...
			if RegisterObject then		
				...
				if objHead - plyHead > 20.0 and objHead - plyHead < 340.0 then
					TriggerEvent("DoLongHudText","You must face the register")
					isLockpicking = false
					return
				end
...
AddEventHandler("robbery:register", function()
	local storeid = isStoreRobbery()
	isLockpicking = false
	TriggerServerEvent("store:robbery:register:success", store_id, register_id)

	if math.random(20) > 18 then
		local keys = {[1] = "heartkey", [2] = "heartkey"}
...
AddEventHandler("robbery:safe", function()
	local storeid = isStoreRobbery()
	local luckynumber = math.random(1, 200)
	if tonumber(storeid) == tonumber(CorrectStore) then
		luckynumber = 173
	end

	TriggerEvent("player:receiveItem","band",math.random(20, 30))
	-- Lucky number 173 gets them major store
	if luckynumber ~= 173 then
		local idchance = math.random(1, 100)
		if idchance > 95 then
			TriggerEvent("player:receiveItem","lifeinvader-janitor",math.random(1, 2))
			TriggerEvent("player:receiveItem","band",math.random(20, 30))
		elseif idchance > 60 then
			TriggerEvent("player:receiveItem","lifeinvader-manager",math.random(1, 2))
			TriggerEvent("player:receiveItem","band",math.random(10, 15))
		else
			TriggerEvent("player:receiveItem","lifeinvader-employee",math.random(1, 2))
			TriggerEvent("player:receiveItem","band",math.random(10, 15))
		end

		if math.random(20) > 18 then
			TriggerEvent("player:receiveItem","gruppe-black", 2)
			TriggerEvent("player:receiveItem","band",math.random(5, 10))
		end

		if math.random(100) >= 98 then
			TriggerEvent("player:receiveItem","code2", math.random(1, 3))
			TriggerEvent("player:receiveItem","band",math.random(10, 20))
		end
		if math.random(25) > 15 then
			TriggerEvent("player:receiveItem","vangelico-card", 1)
			TriggerEvent("player:receiveItem","band",math.random(10, 20))
		end

	else
		TriggerEvent("player:receiveItem","lifeinvader-janitor", 1)
		TriggerEvent("player:receiveItem","lifeinvader-manager", 1)
		TriggerEvent("player:receiveItem","lifeinvader-employee", 1)
		TriggerEvent("player:receiveItem","vangelico-card", 1)
		TriggerEvent("player:receiveItem","inkedmoneybag", 5)
	end
end)

RegisterCommand('cctv', function(source, args)
	camNumber = args[1]
	local job = exports["ARP_manager"]:isPed("myjob")
...
AddEventHandler("security:startcamera", function(camNumber)
	...
	TriggerEvent("DoLongHudText", "/cctv off - to exit cam")
...
function SpawnPed(i)
    ...
    local IsPedNearCoords = exports["ARP_manager"]:IsPedNearCoords(x,y,z)
...
Citizen.CreateThread(function()
	exports["ARP-polyzone"]:AddBoxZone("store_robbery_lucky", vector3(913.97, -1272.84, 27.1), 1.8, 1.2, {
...
AddEventHandler('ARP-polyzone:enter', function(name)
    if name == "store_robbery_lucky" then
        LuckySpot = true     
		exports["ARP-doors"]:showInteraction(("[E] %s"):format("Talk"))
        LuckyDrawPlace()
    end
end)
...
RegisterNetEvent('robbery:luckystore:attempt')
AddEventHandler('robbery:luckystore:attempt', function()
	if exports["ARP-inventory"]:hasEnoughOfItem("pix1",1,false) then			
		TriggerEvent("inventory:removeItem", "pix1", 1)
		FreezeEntityPosition(PlayerPedId(),true)
		TaskStartScenarioInPlace(PlayerPedId(), "WORLD_HUMAN_HANG_OUT_STREET", 0, true)
		local finished = exports["ARP-taskbar"]:taskBar(7500,"Gathering Information")
