                    ...
                    for _, bone in pairs(Bones) do
                        ...
                        if #(bonePos - coords) <= Bones[_]["distance"] then
                            for k , v in ipairs(Bones[_]["job"]) do
                                if v == "all" or v == exports["ARP_manager"]:isPed("myJob") then
    ...
    while true do
        ...
        if entity and entityType ~= 0 then
            if entity ~= CurrentTarget then
                ...
                TriggerEvent('ARP-target:inFront', CurrentTarget, entityType, entityCoords)
            end
        elseif CurrentTarget then
            CurrentTarget = nil
            TriggerEvent('ARP-target:inFront', CurrentTarget)
        end
        Citizen.Wait(idle)
    end
end)
